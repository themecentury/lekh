...
if ( ! function_exists( 'centurylib_sanitize_number' ) ) :
	function centurylib_sanitize_number ( $centurylib_input, $centurylib_setting ) {
	...
	function centurylib_sanitize_repeater($widget_field, $new_field_value ) {
		...
		if(count($new_field_value) && is_array($new_field_value)){
			foreach($new_field_value as $row_index=>$repeater_row){
				$repeater_fields = $widget_field['tcy_widget_field_options'];
				foreach($repeater_fields as $fields_key=>$repeater_field){

					$repeater_field_type = isset($repeater_field['tcy_widget_field_type']) ? $repeater_field['tcy_widget_field_type'] : '';
					$repeater_field_name = isset($repeater_field['tcy_widget_field_name']) ? $repeater_field['tcy_widget_field_name'] : '';
					$repeater_field_value = isset($repeater_row[$repeater_field_name]) ? $repeater_row[$repeater_field_name] : '';
					$sanitize_repeater_value[$row_index][$repeater_field_name] = tcy_widgets_updated_field_value( $repeater_field,  $repeater_field_value);
