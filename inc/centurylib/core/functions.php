...
if(!function_exists('centurylib_taxonomies')){
	...
	function centurylib_taxonomies( $post_types = 'post' ){
		...
		$centurylib_taxonomies[''] = esc_html__('No Filters', 'lekh');
...
if (!function_exists('centurylib_get_image_sizes')){
	...
	function centurylib_get_image_sizes( $disable_image = false ) {
		...
		if ( true == $disable_image ) {
			$centurylib_get_image_sizes['disable'] = esc_html__( 'No Image', 'lekh' );
		}
		foreach ( array( 'thumbnail', 'medium', 'large' ) as $key => $_size ) {
			$centurylib_get_image_sizes[ $_size ] = $_size . ' ('. get_option( $_size . '_size_w' ) . 'x' . get_option( $_size . '_size_h' ) . ')';
		}
		$centurylib_get_image_sizes['full'] = esc_html__( 'full (original)', 'lekh' );
		if ( ! empty( $_wp_additional_image_sizes ) && is_array( $_wp_additional_image_sizes ) ) {
			foreach ($_wp_additional_image_sizes as $key => $size ) {
				$croped_resize = ($size['crop']) ? esc_html__('Croped', 'lekh') : esc_html__('Resize', 'lekh'); 
				$centurylib_get_image_sizes[ $key ] = $key . ' ('. $size['width'] . 'x' . $size['height'] . ') - '.$croped_resize;
			}
		}
...
if (!function_exists('centurylib_post_order')){
	...
	function centurylib_post_order(){
		...
		$centurylib_post_order = array(
			'ASC' => esc_html__( 'ASC', 'lekh' ),
			'DESC' => esc_html__( 'DESC', 'lekh' ),
		);
...
if (!function_exists('centurylib_post_orderby')){
	...
	function centurylib_post_orderby(){
		...
		$centurylib_post_orderby = array(
			'none' => esc_html__( 'None', 'lekh' ),
			'ID' => esc_html__( 'ID', 'lekh' ),
			'author' => esc_html__( 'Author', 'lekh' ),
			'title' => esc_html__( 'Title', 'lekh' ),
			'date' => esc_html__( 'Date', 'lekh' ),
			'modified' => esc_html__( 'Modified Date', 'lekh' ),
			'rand' => esc_html__( 'Random', 'lekh' ),
			'comment_count' => esc_html__( 'Comment Count', 'lekh' ),
			'menu_order' => esc_html__( 'Menu Order', 'lekh' ),
		);
...
if (!function_exists('centurylib_link_target')) {
	...
	function centurylib_link_target(){
		...
		$centurylib_link_target = array(
			''			=> esc_html__('Disable', 'lekh'),
			'_blank'	=> esc_html__('Open with New Tab', 'lekh'),
			'_self'		=> esc_html__('Open with Same Tab', 'lekh'),
			/*'_parent'	=> esc_html__('_parent', 'lekh'),
			'_top'		=> esc_html__('_top', 'lekh'),*/
		);
...
if (!function_exists('centurylib_display_types')) {
	...
	function centurylib_display_types(){
		...
		$centurylib_display_types = array(
			'carousel'=>esc_html__('Carousel', 'lekh'),
			'columns'=>esc_html__('Columns', 'lekh'),
		);
...
if (!function_exists('centurylib_faicon_sizes')){
	...
	function centurylib_faicon_sizes(){
		...
		$fontawesome_icons_size = array(
			''		=> esc_html__('Default', 'lekh'),
			'fa-2x'	=> esc_html__('fa-2x', 'lekh'),
			'fa-3x'	=> esc_html__('fa-3x', 'lekh'),
			'fa-4x'	=> esc_html__('fa-4x', 'lekh'),
			'fa-5x'	=> esc_html__('fa-5x', 'lekh'),
		);
