		...
		if( taxonomy_exists( $tcy_widget_taxonomy_type ) ){
			...
			if( $all_terms ){
				foreach( $all_terms as $single_term ){
					...
					<li>
						<input 
						id="<?php echo esc_attr( $centurywidget->get_field_id($tcy_widget_field_name) ).'_'.$tcy_widget_taxonomy_type.'_'.$teg_term_id; ?>" 
						name="<?php echo esc_attr( $centurywidget->get_field_name($tcy_widget_field_name).'[]' ); ?>" 
						type="checkbox" 
						value="<?php echo $teg_term_id; ?>" 
						<?php checked(in_array($teg_term_id, (array)$tcy_widget_field_value)); ?> 
						/>
						<label for="<?php echo esc_attr( $centurywidget->get_field_id($tcy_widget_field_name) ).'_'.$tcy_widget_taxonomy_type.'_'.$teg_term_id; ?>"><?php echo esc_html( $teg_term_name ).' ('.$single_term->count.')'; ?></label>
					</li>
					<?php
				}
			}else{
				?><span><?php esc_html_e( 'No terms found in this taxonomy', 'lekh' ); ?></span><?php
			}
		}else{
			?><span><?php esc_html_e( 'Selected taxonomy doesn\'t exist', 'lekh' ); ?></span><?php
		}
